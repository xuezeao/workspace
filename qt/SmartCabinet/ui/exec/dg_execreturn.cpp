#include "dg_execreturn.h"
#include "ui_dg_execreturn.h"
...
Dg_ExecReturn::Dg_ExecReturn(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Dg_ExecReturn)
{
    ui->setupUi(this);
...
Dg_ExecReturn::~Dg_ExecReturn()
{
    delete ui;
}

void Dg_ExecReturn::ProcessHandle()
{
    ...
    QSqlQuery query;
    query.exec(QString("select * from T_AgentiaReturnExecute"));
    query.last();
...
void Dg_ExecReturn::ShowPage()
{
    t_exec->setTable("T_AgentiaReturnExecute");
    t_exec->select();
...
//获取抽屉号、位置号、编号
void Dg_ExecReturn::GetDrawerAndPositionNum()
{
    int rowCount = t_exec->rowCount();
...
//显示下一条任务信息
void Dg_ExecReturn::ShowCurrentNeedExecuteTaskInfo(int i)
{
    QString num = "";
...
void Dg_ExecReturn::on_pB_sure_clicked()
{
...
void Dg_ExecReturn::on_CB_return_clicked()
{
...
void Dg_ExecReturn::on_pB_jump_clicked()
{
    isQuitLoopRequestAgentiaStatus = true;
    AllControl(jumpCurrentAct);
}

void Dg_ExecReturn::CreateAndStartThread()
{
    serialPort->moveToThread(threadSerialPort);
    threadSerialPort->start();
}

void Dg_ExecReturn::CreateSerialPort()
{
    emit OpenSerialPort();
}

void Dg_ExecReturn::ErrorMessage(int order)
{
    switch (order) {
...
void Dg_ExecReturn::HandleReply(int order)
{
    if (TASK_ERROR == order)
...
void Dg_ExecReturn::ChangeStatus(QString content)
{
...
//请求当前锁的状态，直至抽屉关闭
void Dg_ExecReturn::RequestLockStatus_Current(int i)
{
...
//已执行任务上报
void Dg_ExecReturn::UploadTask2Server()
{
    QString json_str = "";
    int taskId = -1;
    QString decision = "";
...
//json信息解析
bool Dg_ExecReturn::UnpackageJson_Task(QJsonDocument str)
{
...
void Dg_ExecReturn::ChangeCellContent(int row, int column, QString content)
{
    t_exec->setData(t_exec->index(row, column), content);
    t_exec->submitAll();
}

void Dg_ExecReturn::TextMessageShowContent(QString colorType, QString content)
{
    if (colorType == "green")
...
void Dg_ExecReturn::WriteSQL_NetworkError()
{
...
void Dg_ExecReturn::InitializeStruct()
{
...
void Dg_ExecReturn::UploadWarning(QString json_str, QString warningContent, int model)
{
    bool isSuccess = false;
...
//禁用抽屉和位置
void Dg_ExecReturn::UploadDisableDrawerAndPosition(int model)
{
    bool isSuccess = false;
    QString str = "";
    InitializeStruct();
...
void Dg_ExecReturn::AllControl(int order)
{
    switch (order) {
...
void Dg_ExecReturn::isSaveError(int goal)
{
...
//接受锁的状态
void Dg_ExecReturn::LockStatusSlots(int order)
{
...
void Dg_ExecReturn::waitTime(int time)
{
    QElapsedTimer t;
    t.start();
...
void Dg_ExecReturn::SetTitle(QString name)
{

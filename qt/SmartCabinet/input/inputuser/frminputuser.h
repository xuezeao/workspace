#ifndef FRMINPUTUSER_H
#define FRMINPUTUSER_H
...
class frmInputUser : public QWidget
{
	Q_OBJECT

public:
	explicit frmInputUser(QWidget *parent = 0);
	~frmInputUser();

	//单例模式,保证一个程序只存在一个输入法实例对象
	static frmInputUser *Instance()
	{
		if (!_instance) {
			static QMutex mutex;
	...
	//初始化面板状态,包括字体大小
    void init(QString position, int btnFontSize, int labFontSize,
...
private:
	Ui::frmInputUser *ui;
	static frmInputUser *_instance;  //实例对象
